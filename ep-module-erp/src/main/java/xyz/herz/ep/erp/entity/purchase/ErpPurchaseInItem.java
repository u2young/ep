...
@Getter @Setter
@Entity
@Table(name = "erp_purchase_in_item")
@Erupt(name = "采购入库明细")
public class ErpPurchaseInItem extends BaseModel {

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "in_id")
    private ErpPurchaseIn inDoc;

    @EruptField(views = @View(title = "对应订单明细行ID"),
                edit = @Edit(title = "对应订单明细行ID(可空,若从PO生成则回写订单in_count)",
    ...
    @EruptField(views = @View(title = "批号"),
                edit = @Edit(title = "批号(该产品启用批次管理必填)", inputType = @InputType))
    @Column(length = 100)
    ...
    @EruptField(views = @View(title = "采购单价"),
                edit = @Edit(title = "采购单价", numberType = @NumberType(min = 0)))
