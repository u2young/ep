...
/** ERP 销售客户(和 CRM 客户是两张独立表,业务上需要可以做同步映射)。 */
@Getter @Setter
@Entity
@Table(name = "erp_customer")
@Erupt(name = "销售客户")
public class ErpCustomer extends MetaModelVo {
    ...
    @EruptField(views = @View(title = "客户编码"),
                edit = @Edit(title = "客户编码", notNull = true))
    ...
    @EruptField(views = @View(title = "邮箱"), edit = @Edit(title = "邮箱", inputType = @InputType))
    @Column(length = 100)
    ...
    @EruptField(views = @View(title = "纳税人识别号"), edit = @Edit(title = "纳税人识别号"))
    @Column(length = 50)
    ...
    @EruptField(views = @View(title = "销售员ID(预留,对应用户ID)"),
                edit = @Edit(title = "销售员ID"))
