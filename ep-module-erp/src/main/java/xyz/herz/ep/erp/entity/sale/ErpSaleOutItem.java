...
@Getter @Setter
@Entity
@Table(name = "erp_sale_out_item")
@Erupt(name = "销售出库明细")
public class ErpSaleOutItem extends BaseModel {

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "out_id")
    private ErpSaleOut outDoc;

    @EruptField(views = @View(title = "订单明细行ID"),
                edit = @Edit(title = "订单明细行ID(有则回写销售订单out_count)",
    ...
    @EruptField(views = @View(title = "批号"),
                edit = @Edit(title = "批号(启用批次必填)", inputType = @InputType))
    @Column(length = 100)
