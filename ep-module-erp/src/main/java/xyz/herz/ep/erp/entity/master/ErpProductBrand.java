...
@Getter @Setter
@Entity
@Table(name = "erp_product_brand")
@Erupt(name = "产品品牌")
public class ErpProductBrand extends MetaModelVo {
    ...
    @EruptField(views = @View(title = "品牌编码"),
                edit = @Edit(title = "品牌编码", inputType = @InputType))
    @Column(length = 50, unique = true)
    private String code;

    @EruptField(views = @View(title = "LOGO 图"),
                edit = @Edit(title = "LOGO 图", type = EditType.ATTACHMENT))
    @Column(length = 500)
