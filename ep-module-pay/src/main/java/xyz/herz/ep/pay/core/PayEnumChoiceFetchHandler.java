...
import xyz.herz.ep.pay.enums.PayDictEnums.*;

/**
 * 薪酬管理模块通用字典下拉处理器(复刻 HR/AST/SUP/ERP/PROJ 模板)。
 * <p>用法:
 * <pre>
 * &#64;ChoiceType(fetchHandler = PayEnumChoiceFetchHandler.class, fetchHandlerParams = "SlipStatus")
 * private Integer status;
 * </pre>
 * 支持的 key: EnableStatus / ComponentType / SlipStatus
 */
public class PayEnumChoiceFetchHandler implements ChoiceFetchHandler {

    private static final Map<String, Enum<?>[]> LOOKUP = Map.ofEntries(
        Map.entry("EnableStatus", EnableStatus.values()),
        Map.entry("ComponentType", ComponentType.values()),
        Map.entry("SlipStatus", SlipStatus.values())
    );
    ...
    public List<VLModel> fetch(String[] params) {
        ...
        try {
            ...
            throw new IllegalStateException("PAY enum missing code/label: " + key, roe);
        }
        return r;
    }
}
