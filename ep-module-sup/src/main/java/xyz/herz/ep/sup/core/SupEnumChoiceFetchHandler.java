...
import xyz.herz.ep.sup.enums.SupDictEnums.*;

/**
 * 服务支持模块通用字典下拉处理器(复刻 ERP/PROJ 模板)。
 * <p>用法:
 * <pre>
 * &#64;ChoiceType(fetchHandler = SupEnumChoiceFetchHandler.class, fetchHandlerParams = "IssueStatus")
 * private Integer status;
 * </pre>
 * 支持的 key:
 *  EnableStatus / IssueStatus / IssuePriority / KbStatus
 */
public class SupEnumChoiceFetchHandler implements ChoiceFetchHandler {

    private static final Map<String, Enum<?>[]> LOOKUP = Map.ofEntries(
        Map.entry("EnableStatus", EnableStatus.values()),
        Map.entry("IssueStatus", IssueStatus.values()),
        Map.entry("IssuePriority", IssuePriority.values()),
        Map.entry("KbStatus", KbStatus.values())
    );
    ...
    public List<VLModel> fetch(String[] params) {
        ...
        try {
            ...
            throw new IllegalStateException("SUP enum missing code/label: " + key, roe);
        }
        return r;
    }
}
