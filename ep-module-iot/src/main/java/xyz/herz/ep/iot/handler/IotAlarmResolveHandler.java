...
/**
 * 告警「解决」行按钮:处理中(10)→已解决(20),设置 resolveTime,并写告警日志。
 * <p>强校验:仅 PROCESSING 状态可解决,其他状态抛 IllegalStateException。
 */
@Component
public class IotAlarmResolveHandler implements OperationHandler<Object, Object> {
    ...
    public String exec(List<Object> data, Object formValue, String[] eruptParams) {
        ...
        for (Object row : data) {
            ...
            if (cur == null || cur != AlarmStatus.PROCESSING.code) {
                throw new IllegalStateException(
                    "告警[id=" + a.getId() + "] 当前状态=" + label(cur) + ",仅处理中状态可解决");
            }
            a.setStatus(AlarmStatus.RESOLVED.code);
            a.setResolveTime(LocalDateTime.now());
            em.merge(a);
            writeLog(a, cur, AlarmStatus.RESOLVED.code, "解决");
            ok++;
        }
        return "解决成功 " + ok + " 条";
    }
