...
/**
 * 告警「处理」行按钮:待处理(0)→处理中(10),并写告警日志。
 * <p>强校验:仅 PENDING 状态可处理,其他状态抛 IllegalStateException。
 */
@Component
public class IotAlarmProcessHandler implements OperationHandler<Object, Object> {
    ...
    public String exec(List<Object> data, Object formValue, String[] eruptParams) {
        ...
        for (Object row : data) {
            ...
            if (cur == null || cur != AlarmStatus.PENDING.code) {
                throw new IllegalStateException(
                    "告警[id=" + a.getId() + "] 当前状态=" + label(cur) + ",仅待处理状态可处理");
            }
            a.setStatus(AlarmStatus.PROCESSING.code);
            em.merge(a);
            writeLog(a, cur, AlarmStatus.PROCESSING.code, "处理");
            ok++;
        }
        return "处理成功 " + ok + " 条";
    }
