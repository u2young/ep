...
/**
 * 库存管理模块状态机 DataProxy 基类(复刻 PUR/HR/PAY/QAL 模板,避免跨模块依赖)。
 * <p>禁止表单直接编辑 status,只能通过 @RowOperation 的「提交/取消」等按钮改状态。
 *
 * <p>默认行为:DRAFT(0) 状态下允许用户保存时 status 仍为 0;其他值一律禁止。
 */
public abstract class StkStateDataProxy<T> implements DataProxy<T> {
    ...
    public void beforeUpdate(T target) {
        ...
        throw new IllegalArgumentException(
            "状态字段 " + stateFieldName() + " 禁止在表单里直接修改,请通过行按钮(提交/取消)变更状态。"
        );
    }
}
