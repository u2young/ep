...
/**
 * 售后「同意」行按钮处理器:10 申请 → 20 同意。
 */
@Component
public class MallAfterSaleAgreeHandler implements OperationHandler<Object, Object> {

    public static final String CODE = "mall.aftersale.agree";
        ...
        for (Object row : data) {
            try {
                ...
                if (a.getStatus() == null || a.getStatus() != AfterSaleStatus.APPLY.code) {
                    throw new IllegalStateException("当前状态不允许同意: " + AftersaleLabel.of(a.getStatus()));
                }
                a.setStatus(AfterSaleStatus.AGREED.code);
    ...
    static final class AftersaleLabel {
        static String of(Integer code) {
            if (code == null) return "null";
            for (AfterSaleStatus s : AfterSaleStatus.values()) if (s.code == code) return s.label;
            return "UNKNOWN(" + code + ")";
        }
    }
}
