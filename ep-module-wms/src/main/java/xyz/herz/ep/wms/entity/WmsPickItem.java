...
@Getter @Setter
@Entity
@Table(name = "wms_pick_item")
@Erupt(name = "拣货明细")
public class WmsPickItem extends BaseModel {

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "pick_id")
    private WmsPick pick;
    ...
    @EruptField(views = @View(title = "拣货数量"),
                edit = @Edit(title = "拣货数量", notNull = true,
                    numberType = @NumberType(min = 0)))
    @Column(name = "pick_qty", nullable = false)
