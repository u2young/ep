...
/**
 * 落地页状态机 DataProxy 基类(参考 MallStateDataProxy,独立实现避免跨模块依赖)。
 * <p>禁止表单直接编辑状态字段,只能通过 @RowOperation 的行按钮(发布/下线)变更状态。
 *
 * <p>默认行为:仅允许状态字段的初始值(0 或 null)被表单直接写入,其它值一律禁止。
 * 子类可覆写 {@link #allowedDirectEditStatuses()} 放宽。
 */
public abstract class LandingStateDataProxy<T> implements DataProxy<T> {
