...
/**
 * 财务模块状态机 DataProxy 基类(复刻 ERP 模板 ErpStateDataProxy,避免跨模块依赖)。
 * <p>禁止表单直接编辑 status,只能通过 @RowOperation 的「提交/取消/审核」等按钮改状态。
 *
 * <p>默认行为:DRAFT(0) 状态下允许用户保存时 status 仍为 DRAFT;其他值一律禁止。
 * 需要放松时子类覆写 {@link #allowedDirectEditStatuses()}。
 */
public abstract class FinStateDataProxy<T> implements DataProxy<T> {
    ...
    public void beforeUpdate(T target) {
        ...
        throw new IllegalArgumentException(
            "状态字段 " + stateFieldName() + " 禁止在表单里直接修改,请通过行按钮(提交/取消/审核)变更状态。"
        );
    }
}
